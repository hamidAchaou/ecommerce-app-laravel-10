    ...
    <div class="relative" x-data="{ open: false }">
        ...
        @else
            <div class="space-x-4 flex items-center">
                <a href="{{ route('login') }}"
                   class="px-4 py-2 border border-indigo-600 text-indigo-600 rounded-md hover:bg-indigo-600 hover:text-white transition font-medium">
                    <i class="fas fa-sign-in-alt mr-1"></i> Login
                </a>
                @if (Route::has('register'))
                    <a href="{{ route('register') }}"
                       class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition font-medium flex items-center">
