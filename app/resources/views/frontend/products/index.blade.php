...
@section('meta_description', 'Browse our curated selection of Moroccan crafts, pottery, weaving, woodwork, jewelry, and leatherwork. Filter by price and category for an easier shopping experience.')
        ...
        <div class="bg-white rounded-2xl shadow-lg p-6 sticky top-20 hover:shadow-2xl transition-shadow duration-300">
            ...
            <form action="{{ route('products.index') }}" method="GET" class="flex gap-2">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search products..."
                    class="w-full rounded-xl border-gray-300 shadow-sm focus:ring-red-500 focus:border-red-500 px-4 py-2 text-sm">
        ...
        <div class="bg-white rounded-2xl shadow-lg p-6 sticky top-24 hover:shadow-2xl transition-shadow duration-300">
            ...
            <form action="{{ route('products.index') }}" method="GET" class="space-y-4">
                ...
                <div class="relative h-2 bg-red-200 rounded-full">
                    <input type="range" min="0" max="500" value="{{ request('min') ?? 0 }}" id="minSlider"
                        class="absolute w-full h-2 bg-transparent appearance-none pointer-events-none">
                    <input type="range" min="0" max="500" value="{{ request('max') ?? 500 }}" id="maxSlider"
                        class="absolute w-full h-2 bg-transparent appearance-none pointer-events-none">
                ...
                <button type="submit" class="w-full bg-red-600 text-white font-semibold py-2 rounded-xl shadow hover:bg-red-700 transition">
                    Apply Filter
                </button>
            </form>
        </div>
