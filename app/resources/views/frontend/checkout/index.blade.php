        ...
        <div x-data="checkoutPage()" class="lg:flex lg:gap-12">
                ...
                <div class="bg-white p-6 rounded-xl shadow">
                    ...
                    @if ($client && $client->phone && $client->address && $client->country_id && $client->city_id)
                        ...
                        <p class="mt-3 text-sm text-gray-500">
                            Not correct?
                            <a href="{{ route('profile.edit') }}" class="text-morocco-red font-medium hover:underline">Update your profile</a>
                        </p>
                    @else
                        {{-- Show shipping form --}}
                        <form id="checkout-form" class="space-y-5">
                            @csrf
                            <x-frontend.form.input id="name" name="name" label="Full Name" value="{{ old('name', auth()->user()?->name) }}" required />
                            <x-frontend.form.input id="phone" name="phone" label="Phone Number" value="{{ old('phone') }}" required />
                            <x-frontend.form.textarea id="address" name="address" label="Address" rows="3" required>{{ old('address') }}</x-frontend.form.textarea>

                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <x-frontend.form.select id="country_id" name="country_id" label="Country" x-model="selectedCountry" required>
                                ...
                                <x-frontend.form.select id="city_id" name="city_id" label="City" x-model="selectedCity" :disabled="!selectedCountry" required>
                            ...
                            <x-frontend.form.textarea id="notes" name="notes" label="Order Notes (optional)" rows="2">{{ old('notes') }}</x-frontend.form.textarea>
                        </form>
                    @endif
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <template x-if="cartItems.length === 0">
                        <div class="text-center py-16">
                            ...
                            <x-frontend.button.button-primary :href="route('products.index')">Continue Shopping</x-frontend.button.button-primary>
                        </div>
                    </template>
                </div>
            </div>
            ...
            <aside class="lg:w-1/3 mt-6 lg:mt-0 bg-white rounded-xl shadow p-6 h-fit">
                ...
                <div class="mt-6 space-y-3">
                    {{-- Stripe Payment --}}
                    <x-frontend.button.button-primary type="button" @click="payWithStripe" class="w-full flex items-center justify-center gap-2 relative disabled:opacity-70 disabled:cursor-not-allowed transition-all duration-200" x-bind:disabled="isLoadingPayment">
                        <template x-if="isLoadingPayment">
                            <svg class="animate-spin h-5 w-5 text-white absolute left-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    ...
                    {{-- Clear Cart --}}
                    <button @click="clearCart()" class="w-full py-3 bg-red-600 text-white font-semibold rounded-xl hover:bg-red-700 transition" x-show="cartItems.length > 0">
                        Clear Cart
                    </button>
...
function checkoutPage() {
    return {
        ...
        async payWithStripe() {
            ...
            if (this.cartItems.length === 0) { alert('Cart is empty'); return; }
            ...
            } else { alert('Client info missing'); return; }
            ...
            try {
                ...
                if (data.error) { alert(data.error); this.isLoadingPayment=false; return; }
